...
int main(int argc, char * argv[])
{
	try
	{
		...
		unsigned int const maxclip = arginfo.getValue<unsigned int>("maxclip",0);
		...
		::libmaus2::bambam::BamHeader::unique_ptr_type genuphead(
			libmaus2::bambam::BamHeaderUpdate::updateHeader(arginfo,header,"clipfilter",std::string(PACKAGE_VERSION))
		);
		...
		while ( dec.readAlignment() )
			if ( algn.getFrontSoftClipping() <= maxclip && algn.getBackSoftClipping() <= maxclip )
